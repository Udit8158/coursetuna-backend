const { verifyAdminAuthToken, verifyUserAuthToken } = require("../../utils/jwtHelper");
...
const authorizeAsAdmin = (req, res, next) => {
    try {
        ...
        const result = verifyAdminAuthToken(authToken);
...
const authorizeAsUser = (req, res, next) => {
    try {
        ...
        const result = verifyUserAuthToken(authToken);
        if (!result.role == "user")
